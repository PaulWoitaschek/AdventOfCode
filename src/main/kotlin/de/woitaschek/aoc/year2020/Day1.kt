...
object Day1 : Puzzle(2020, 1) {
  ...
  private fun solve(input: String, permutations: (List<Long>) -> Sequence<List<Long>>): Long =
    input.lines().filter(String::isNotEmpty).map { it.toLong() }
      .let(permutations)
      .first { it.sum() == 2020L }
      .reduce(Long::times)
